...
import com.varitas.gokulpos.activity.TenderActivity
import com.varitas.gokulpos.databinding.FragmentDTenderBinding
...
class TenderPopupDialog : BaseDialogFragment() {
    ...
    override fun onViewCreated(view : View, savedInstanceState : Bundle?) {
        ...
        if(arguments != null) {
            if(arguments!!.getParcelable<Tender>(Default.TENDER) != null) {
                tenderDetails = if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) arguments?.getParcelable(Default.TENDER, Tender::class.java)!!
                else arguments?.getParcelable(Default.TENDER)!!
            }
        }
        initData()
        postInitViews()
                ...
                if(tenderDesc.isNotEmpty()) {
                    if(tenderDetails == null) {
                        ...
                        TenderActivity.Instance.viewModel.updateTender(req).observe(this@TenderPopupDialog) {
                            if(it) onButtonClickListener?.onButtonClickListener(Enums.ClickEvents.UPDATE, Tender(name = tenderDesc, sCardType = if(cardType.value!! > 0) cardType.label else ""))
                        }
                    }
