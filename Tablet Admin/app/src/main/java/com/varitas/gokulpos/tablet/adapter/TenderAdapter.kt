...
import com.varitas.gokulpos.tablet.databinding.AdapterTenderBinding
import com.varitas.gokulpos.tablet.response.Tender

class TenderAdapter(private var onClickListener : (Tender, Int, Boolean) -> Unit) : RecyclerView.Adapter<TenderAdapter.ViewHolder>() {

    var list = ArrayList<Tender>()
    var filteredList = ArrayList<Tender>()

    fun setList(title : List<Tender>) {
        list.clear()
        list.addAll(title)
        filteredList.clear()
    ...
    override fun onCreateViewHolder(parent : ViewGroup, viewType : Int) : ViewHolder {
        ...
        val binding = AdapterTenderBinding.inflate(inflater, parent, false)
        ...
        return object : Filter() {
            override fun performFiltering(charSequence : CharSequence) : FilterResults {
                ...
                filteredList = if(searchString.isEmpty()) {
                    ...
                    val tempFilteredList = ArrayList<Tender>()
            ...
            @SuppressLint("NotifyDataSetChanged") @Suppress("UNCHECKED_CAST") override fun publishResults(charSequence : CharSequence?, filterResults : FilterResults) {
                filteredList = filterResults.values as ArrayList<Tender>
                notifyDataSetChanged()
            }
        }
    } //endregion

    inner class ViewHolder(private val binding : AdapterTenderBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(data : Tender, position : Int) {
            binding.apply {
                tender = data
                pos = position + 1
