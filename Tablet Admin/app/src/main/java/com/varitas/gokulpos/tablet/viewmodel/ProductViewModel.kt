...
@HiltViewModel class ProductViewModel @Inject constructor(private val productsRepository : ProductsRepository) : ViewModel() {
    ...
    //region To fetch taxes
    fun fetchTaxes() : MutableLiveData<List<Tax>> {
        val tax = MutableLiveData<List<Tax>>()
        viewModelScope.launch(Dispatchers.IO) {
            try {
                showProgress.postValue(true)
                productsRepository.fetchTax(storeId!!, { response->
                    showProgress.postValue(false)
                    if(response.status == Default.SUCCESS_API) {
                        if(response.data != null)
                        tax.postValue(response.data!!.filter { it.status == Default.ACTIVE })
                    }
                    else tax.postValue(ArrayList())
                }, { t->
    ...
    fun fetchCategories() : MutableLiveData<List<DropDownCategories>> {
        val list = MutableLiveData<List<DropDownCategories>>()
        viewModelScope.launch(Dispatchers.IO) {
            try {
                showProgress.postValue(true)
                productsRepository.fetchCategories(storeId!!, { response->
                    showProgress.postValue(false)
                    if(response.status == Default.SUCCESS_API) {
                        if(response.data != null)
                        list.postValue(response.data!!.sortedBy { it.categoryName })
                    }
                    else list.postValue(ArrayList())
                }, { t->
    ...
    //region To fetch sub categories
    fun fetchSubCategories() : MutableLiveData<List<Category>> {
        val list = MutableLiveData<List<Category>>()
        viewModelScope.launch(Dispatchers.IO) {
            try {
                showProgress.postValue(true)
                productsRepository.fetchSubCategories({ response->
                    showProgress.postValue(false)
                    if(response.status == Default.SUCCESS_API) list.postValue(response.data)
                    else list.postValue(ArrayList())
                }, { t->
                    Log.e("Sub Categories", "onFailure: ", t)
                    showProgress.postValue(false)
                })
            } catch(ex : Exception) {
                Utils.printAndWriteException(ex)
    ...
    //region To make shortcut
    fun makeShortcut(id: Int): MutableLiveData<Boolean> {
        val isSuccess = MutableLiveData<Boolean>()
        viewModelScope.launch(Dispatchers.IO) {
            try {
                showProgress.postValue(true)
                productsRepository.makeShortcut(id, { response ->
                    showProgress.postValue(false)
                    isSuccess.postValue(response.status == Default.SUCCESS_API)
                    errorMsg.postValue(response.message)
                }, { t ->
                    Log.e("Make Shortcut", "onFailure: ", t)
                    showProgress.postValue(false)
                })
            } catch (ex: Exception) {
                Utils.printAndWriteException(ex)
    ...
    //region To fetch products
    fun fetchProductsFilterWise(departmentId: Int, categoryId: Int, brandId: Int, itemTypeId: Int) : MutableLiveData<List<ProductList>> {
        val products = MutableLiveData<List<ProductList>>()
        viewModelScope.launch(Dispatchers.IO) {
            try {
                showProgress.postValue(true)
                productsRepository.fetchProductsFilterWise(storeId!!, departmentId, categoryId, brandId, itemTypeId,{ response->
                    showProgress.postValue(false)
                    if(response.status == Default.SUCCESS_API) products.postValue(response.data)
                    else products.postValue(ArrayList())
                }, { t->
